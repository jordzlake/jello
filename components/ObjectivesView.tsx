...
const PILL_COLORS = [
  'rgba(111,95,255,.25)','rgba(0,210,106,.22)','rgba(255,95,160,.22)',
  'rgba(0,198,255,.22)','rgba(249,115,22,.22)','rgba(234,179,8,.22)',
  'rgba(168,85,247,.22)','rgba(20,184,166,.22)','rgba(239,68,68,.22)',
  'rgba(255,255,255,.09)','rgba(255,255,255,.04)','rgba(30,30,50,.8)',
];
...
function getCachedImage(url: string): string | null {
  ...
  try { return localStorage.getItem(icKey(url)); } catch { return null; }
}
    ...
    <div style={{ display:'flex', height:'100%', minHeight:0, overflow:'hidden', position:'relative' }}>
      ...
      {isMobile && sideOpen && (
        <div onClick={() => setSideOpen(false)}
          style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.55)', zIndex:90, backdropFilter:'blur(3px)' }}/>
      )}
      ...
      <aside style={{
        ...
        <div style={{ padding:'16px 14px 10px', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
          ...
          <div style={{ display:'flex', gap:5 }}>
            <button onClick={() => setAddObjOpen(true)} style={iconBtn}><i className="fa-solid fa-plus"/></button>
            {isMobile && <button onClick={() => setSideOpen(false)} style={iconBtn}><i className="fa-solid fa-xmark"/></button>}
          </div>
        </div>
        <div style={{ flex:1, overflowY:'auto', padding:'0 8px 16px' }}>
          ...
          {store.objectives.map(obj => {
            ...
            return (
              <div key={obj.id}
                onClick={() => { setSelectedId(obj.id); if (isMobile) setSideOpen(false); }}
                style={{
                  ...
                  transition:'all .22s',
                  boxShadow: active ? `0 0 18px rgba(${rgb(obj.color)},.22)` : 'none',
                }}>
                <div style={{ position:'absolute', left:0, top:0, bottom:0, width:`${pct}%`, background:`linear-gradient(90deg,rgba(${rgb(obj.color)},.22),rgba(${rgb(obj.color2)},.1))`, transition:'width .7s cubic-bezier(.4,0,.2,1)', pointerEvents:'none' }}/>
                <div style={{ position:'relative', zIndex:1 }}>
                  ...
                  <div style={{ fontSize:'.68rem', color:'var(--muted)' }}>{doneN}/{total} components</div>
                </div>
              </div>
            );
          })}
      ...
      <main style={{ flex:1, position:'relative', overflow:'hidden', minHeight:0, display:'flex', flexDirection:'column' }}>
        {/* Mobile top bar */}
        {isMobile && (
          <div style={{ padding:'10px 14px', display:'flex', alignItems:'center', gap:10, borderBottom:'1px solid var(--border)', background:'rgba(11,11,19,.7)', flexShrink:0 }}>
      ...
      {addCompOpen && selected && (
        <AddCompModal onClose={() => setAddCompOpen(false)}
          onCreate={(text,notes) => { store.addComponent(selected.id, text); setAddCompOpen(false); }}/>
      )}
      {editCompId && selected && (() => {
...
// ─────────────────────────────────────────────────────────────────────
function MindMap({ obj, isMobile, onToggle, onEditComp, onDeleteComp, onUpdateComp, onAddComp, onEditObj, onDeleteObj }: {
  obj: Objective; isMobile: boolean;
  onToggle:(id:string)=>void; onEditComp:(id:string)=>void;
  onDeleteComp:(id:string)=>void; onUpdateComp:(id:string,patch:Partial<ObjComponent>)=>void;
  onAddComp:()=>void; onEditObj:()=>void; onDeleteObj:()=>void;
}) {
  ...
  return (
    ...
    >
      ...
      {/* Scrollable column */}
      <div style={{ position:'relative', zIndex:2, display:'flex', flexDirection:'column', alignItems:'center', padding: isMobile ? '16px 12px 32px' : '24px 24px 40px', gap:0, minHeight:'100%' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:10, width:'100%', maxWidth:340, marginTop: isMobile ? 16 : 24 }}>
          ...
          <div ref={centerRef} style={{
            ...
            {pulse && <div style={{ position:'absolute', inset:-2, borderRadius:24, border:`2.5px solid ${obj.color}`, animation:'objPulse .7s ease-out', pointerEvents:'none', zIndex:20 }}/>}
            {overCenter && <div style={{ position:'absolute', inset:0, background:'linear-gradient(90deg,transparent,rgba(255,255,255,.08),transparent)', animation:'shimmer .85s linear infinite', pointerEvents:'none', zIndex:10 }}/>}

            {/* Header */}
            <div style={{ padding:'18px 18px 12px', borderBottom:'1px solid rgba(255,255,255,.07)' }}>
              <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:8, marginBottom:10 }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontFamily:'Syne,sans-serif', fontWeight:900, fontSize: isMobile ? '.95rem' : '1.05rem', lineHeight:1.22, background:`linear-gradient(135deg,${obj.color},${obj.color2})`, WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', marginBottom:3 }}>
                    {obj.title}
                  </div>
                  {obj.description && (
              ...
              <div style={{ marginTop:10, height:4, background:'rgba(255,255,255,.07)', borderRadius:2, overflow:'hidden' }}>
                <div style={{ height:'100%', width:`${pct}%`, background:`linear-gradient(90deg,${obj.color},${obj.color2})`, borderRadius:2, transition:'width .8s cubic-bezier(.4,0,.2,1)' }}/>
              </div>
            </div>

            {/* Done list */}
            <div style={{ padding:'10px 14px 14px', maxHeight: isMobile ? 180 : 240, overflowY:'auto' }}>
              {done.length === 0 ? (
                <div style={{ textAlign:'center', padding:'14px 0', fontSize:'.72rem', color:'var(--muted)', fontStyle:'italic' }}>
        ...
        return (
          <div style={{
            ...
            <div style={{
              ...
              background: (comp as any).bgColor || `rgba(${rgb(obj.color)},.35)`,
              ...
              <div style={{ width:9,height:9,borderRadius:'50%',background:`linear-gradient(135deg,${obj.color},${obj.color2})`,flexShrink:0 }}/>
              <span style={{ fontSize:'.8rem', fontWeight:700 }}>{comp.text}</span>
              {comp.notes && <span style={{ fontSize:'.65rem', color:'var(--muted)', maxWidth:100, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{comp.notes}</span>}
              <span style={{ fontSize:'.65rem', color:obj.color, fontWeight:800 }}>+{stepPct}%</span>
            </div>
          </div>
        );
      })()}
...
function PillNode({ comp, color, color2, stepPct, isDragging, isJustDone, pillRef, onStartDrag, onEdit, onDelete, onColorChange }: {
  ...
  return (
    ...
    >
      {/* Color dot */}
      <div data-nd="1" onClick={() => setColorOpen(v=>!v)}
        style={{ width:10,height:10,borderRadius:'50%',background:`linear-gradient(135deg,${color},${color2})`,flexShrink:0,cursor:'pointer' }}
        title="Change colour"/>

      <div style={{ display:'flex', flexDirection:'column', minWidth:0 }}>
        <span style={{ fontSize:'.78rem', fontWeight:700, lineHeight:1.2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
          {comp.text}
        </span>
        {comp.notes && (
          <span style={{ fontSize:'.62rem', color:'var(--muted)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:120 }}>
            {comp.notes}
          </span>
        )}
      </div>

      <span style={{ fontSize:'.6rem', color, fontWeight:800, flexShrink:0 }}>+{stepPct}%</span>

      {hov && (
        <div data-nd="1" style={{ display:'flex', gap:3 }}>
          <PillBtn c="rgba(255,255,255,.15)" title="Edit"   onClick={onEdit}><i className="fa-solid fa-pen"   style={{fontSize:'.5rem'}}/></PillBtn>
          <PillBtn c="rgba(239,68,68,.3)"    title="Delete" onClick={onDelete}><i className="fa-solid fa-trash" style={{fontSize:'.5rem'}}/></PillBtn>
        </div>
      )}

      {/* Colour picker */}
      {colorOpen && (
        <div data-nd="1" onClick={e=>e.stopPropagation()}
          style={{ position:'absolute', top:'calc(100% + 8px)', left:0, zIndex:200, background:'rgba(14,14,22,.98)', border:'1px solid var(--border)', borderRadius:12, padding:8, display:'flex', flexWrap:'wrap', gap:5, width:148, boxShadow:'0 16px 48px rgba(0,0,0,.65)', animation:'menuIn .14s ease' }}>
          {PILL_COLORS.map((c,i) => (
            <div key={i} onClick={() => { onColorChange(c); setColorOpen(false); }}
              style={{ width:24,height:24,borderRadius:6,background:c,border:`2px solid ${c===bgColor?'#fff':'transparent'}`,cursor:'pointer',transition:'transform .12s' }}
...
// ─────────────────────────────────────────────────────────────────────
function DoneRow({ comp, color, isDragging, onStartDrag, onDrop, onEdit, onDelete }: {
  ...
  const handlePointerUp = (e: React.PointerEvent) => onDrop(e.clientX, e.clientY);
  ...
  return (
    <div
      ...
      <i className="fa-solid fa-check" style={{ fontSize:'.56rem', color, flexShrink:0 }}/>
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ fontSize:'.74rem', color:'var(--muted)', textDecoration:'line-through', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{comp.text}</div>
        {comp.notes && <div style={{ fontSize:'.6rem', color:'var(--muted)', opacity:.6, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{comp.notes}</div>}
      </div>
      {hov && (
        <div style={{ display:'flex', gap:3, alignItems:'center' }}>
          <span style={{ fontSize:'.54rem', color:'var(--muted)', whiteSpace:'nowrap' }}>drag out</span>
          <PillBtn c="rgba(255,255,255,.12)" title="Edit"   onClick={onEdit}><i className="fa-solid fa-pen"   style={{fontSize:'.46rem'}}/></PillBtn>
          <PillBtn c="rgba(239,68,68,.25)"   title="Delete" onClick={onDelete}><i className="fa-solid fa-trash" style={{fontSize:'.46rem'}}/></PillBtn>
        </div>
      )}
    </div>
  );
...
function ObjMenu({ onEdit, onDelete }: { onEdit:()=>void; onDelete:()=>void }) {
  ...
  return (
    <div style={{ position:'relative', flexShrink:0 }}>
      ...
      {open && (
        <div onClick={()=>setOpen(false)} style={{ position:'absolute', right:0, top:'calc(100% + 6px)', background:'rgba(14,14,22,.98)', border:'1px solid var(--border)', borderRadius:12, padding:4, minWidth:148, zIndex:50, boxShadow:'0 16px 48px rgba(0,0,0,.65)', animation:'menuIn .15s ease' }}>
...
function ProgressRing({ pct,color,color2,size }:{ pct:number;color:string;color2:string;size:number }) {
  ...
  return (
    <svg width={size} height={size} style={{flexShrink:0}}>
      ...
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={`url(#${id})`} strokeWidth={4.5}
        strokeLinecap="round"
        strokeDasharray={`${pct/100*circ} ${circ-pct/100*circ}`}
        strokeDashoffset={circ*.25}
        style={{transition:'stroke-dasharray .8s cubic-bezier(.4,0,.2,1)'}}/>
      <text x={size/2} y={size/2+1} textAnchor="middle" dominantBaseline="middle" fontSize={9} fontWeight={700} fill="var(--text)" fontFamily="DM Sans,sans-serif">{pct}%</text>
    </svg>
  );
}
...
function AddObjModal({onClose,onCreate}:{onClose:()=>void;onCreate:(t:string,d:string,c1:string,c2:string)=>void}) {
  ...
  const go=()=>{if(!title.trim())return;const[c1,c2]=ACCENT_PAIRS[pi];onCreate(title.trim(),desc.trim(),c1,c2);};
  return(<Ov onClose={onClose}><MB title="New Objective" onClose={onClose}>
    <Lb>Title</Lb><In value={title} set={setTitle} ph="e.g. Launch product v2" af onEnt={go}/>
    <Lb>Description (optional)</Lb><In value={desc} set={setDesc} ph="What does success look like?"/>
    <Lb>Color</Lb><Pal pi={pi} setPi={setPi}/>
    <Rw><Gb onClick={onClose}>Cancel</Gb><Pb onClick={go} c={ACCENT_PAIRS[pi][0]}>Create</Pb></Rw>
  </MB></Ov>);
}
function AddCompModal({onClose,onCreate}:{onClose:()=>void;onCreate:(t:string,n:string)=>void}) {
  const [text,setText]=useState('');const [notes,setNotes]=useState('');
  const go=()=>text.trim()&&onCreate(text.trim(),notes.trim());
  return(<Ov onClose={onClose}><MB title="Add Component" onClose={onClose}>
    <Lb>Name</Lb><In value={text} set={setText} ph="e.g. Design mockups" af onEnt={go}/>
    <Lb>Description (optional)</Lb><In value={notes} set={setNotes} ph="Brief description…"/>
    <Rw><Gb onClick={onClose}>Cancel</Gb><Pb onClick={go}>Add</Pb></Rw>
  </MB></Ov>);
}
function EditCompModal({comp,onClose,onSave}:{comp:ObjComponent;onClose:()=>void;onSave:(p:Partial<ObjComponent>)=>void}) {
  ...
  return(<Ov onClose={onClose}><MB title="Edit Component" onClose={onClose}>
    <Lb>Name</Lb><In value={text} set={setText} af onEnt={()=>onSave({text:text.trim(),notes})}/>
    <Lb>Description</Lb>
    <textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Brief description…"
    ...
    <Rw><Gb onClick={onClose}>Cancel</Gb><Pb onClick={()=>onSave({text:text.trim(),notes})}>Save</Pb></Rw>
  </MB></Ov>);
}
function EditObjModal({obj,onClose,onSave}:{obj:Objective;onClose:()=>void;onSave:(p:Partial<Objective>)=>void}) {
  ...
  return(<Ov onClose={onClose}><MB title="Edit Objective" onClose={onClose}>
    <Lb>Title</Lb><In value={title} set={setTitle} af/>
    <Lb>Description</Lb><In value={desc} set={setDesc}/>
    <Lb>Color</Lb><Pal pi={pi} setPi={setPi}/>
    <Rw><Gb onClick={onClose}>Cancel</Gb><Pb onClick={()=>onSave({title:title.trim(),description:desc,color:ACCENT_PAIRS[pi][0],color2:ACCENT_PAIRS[pi][1]})} c={ACCENT_PAIRS[pi][0]}>Save</Pb></Rw>
  </MB></Ov>);
}
...
function Rw({children}:{children:React.ReactNode}){return <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>{children}</div>;}
function Gb({children,onClick}:{children:React.ReactNode;onClick:()=>void}){return <button onClick={onClick} style={{background:'var(--surface)',border:'1px solid var(--border)',color:'var(--muted)',fontFamily:'DM Sans,sans-serif',fontSize:'.8rem',padding:'8px 16px',borderRadius:8,cursor:'pointer'}}>{children}</button>;}
function Pb({children,onClick,c='var(--accent)'}:{children:React.ReactNode;onClick:()=>void;c?:string}){return <button onClick={onClick} style={{background:c,border:'none',color:'#fff',fontFamily:'DM Sans,sans-serif',fontSize:'.8rem',fontWeight:600,padding:'8px 18px',borderRadius:8,cursor:'pointer'}}>{children}</button>;}
function PillBtn({children,onClick,c,title}:{children:React.ReactNode;onClick:()=>void;c:string;title?:string}){return <button title={title} onClick={e=>{e.stopPropagation();onClick();}} style={{background:c,border:'none',borderRadius:5,width:20,height:20,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',flexShrink:0}}>{children}</button>;}
function MI({icon,label,danger,onClick}:{icon:string;label:string;danger?:boolean;onClick:()=>void}){return <div onClick={onClick} className="menu-item-hover" style={{padding:'8px 12px',borderRadius:7,fontSize:'.76rem',cursor:'pointer',display:'flex',alignItems:'center',gap:8,color:danger?'#ff6b6b':'var(--text)'}}><i className={`fa-solid ${icon}`} style={{width:12}}/>{label}</div>;}

const iconBtn:React.CSSProperties={background:'none',border:'1px solid var(--border)',borderRadius:7,color:'var(--muted)',width:26,height:26,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'.7rem'};
