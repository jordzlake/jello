  ...
  return (
    <div>
      ...
      >
        ...
        >
          {fmtDateFull(startDay)} — {fmtDateFull(endDay)}
        </span>
        <button onClick={() => onNav(1)} style={navBtn}>
                    ...
                    <th
                      ...
                      style={{
                        ...
                        background: isT
                          ? "rgba(111,95,255,.22)"
                          : "rgba(18,18,28,.95)",
                        color: isT ? "var(--accent)" : "var(--muted)",
                        whiteSpace: "nowrap",
                        fontSize: zoom === "week" ? ".66rem" : ".58rem",
                      }}
                    >
                      {lbl}
                    </th>
              ...
              {Object.values(groups).map((grp: Grp, gi) => [
                <tr key={`g${gi}`}>
                  <td
                    colSpan={days + 1}
                    style={{
                      background: `linear-gradient(90deg,${grp.pal.c1}28,transparent)`,
                      padding: "7px 14px",
                      fontFamily: "Syne,sans-serif",
                      fontWeight: 700,
                      fontSize: ".8rem",
                ...
                ...grp.tasks.map((t: any, ri: number) => {
                  ...
                  const leftPx = Math.floor(
                    ((visStart - startMs) / mspd) * cellW,
                  );
                  const widthPx = Math.max(
                    Math.floor(((visEnd - visStart) / mspd) * cellW),
                    6,
                  );
                  ...
                  const todayOff = Math.round(
                    ((today.getTime() - startMs) / mspd) * cellW,
                  );
                  ...
                  return (
                    <tr
                      ...
                      onMouseEnter={(e) =>
                        (e.currentTarget.style.background = "rgba(18,18,28,.8)")
                      }
                      onMouseLeave={(e) =>
