...
export default function GanttView({ D, onSetZoom, onNav, onGoToday, onOpenDate }: Props) {
  ...
  const startDay = new Date(today); startDay.setDate(startDay.getDate()+offset);
  const endDay = new Date(startDay); endDay.setDate(endDay.getDate()+days-1);

  const colDates: Date[] = [];
  for(let i=0;i<days;i++){const d=new Date(startDay);d.setDate(d.getDate()+i);colDates.push(d);}
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16,flexWrap:'wrap'}}>
        <div style={{fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:'1.35rem',background:'linear-gradient(135deg,#00d26a,#ff81f5)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>
        ...
        <span style={{fontSize:'.78rem',color:'var(--muted)',whiteSpace:'nowrap'}}>{fmtDateFull(startDay)} — {fmtDateFull(endDay)}</span>
          ...
          <table style={{minWidth:'100%',borderCollapse:'collapse',tableLayout:'fixed',width:200+gridW}}>
            ...
            <tbody>
              {Object.values(groups).map((grp:Grp,gi)=>[
                <tr key={`g${gi}`}>
                  <td colSpan={days+1} style={{background:`linear-gradient(90deg,${grp.pal.c1}28,transparent)`,padding:'7px 14px',fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:'.8rem',borderBottom:'1px solid var(--border)'}}>
                    {grp.title}
                  </td>
                </tr>,
                ...grp.tasks.map((t:any,ri:number)=>{
                  const tStart = t.startDate?new Date(t.startDate+'T00:00:00'):new Date(t.endDate+'T00:00:00');
                  ...
                  const todayOff = Math.round((today.getTime()-startMs)/mspd*cellW);
