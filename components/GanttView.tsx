...
export default function GanttView({ D, onSetZoom, onNav, onGoToday, onOpenDate }: Props) {
  ...
  const mspd   = 86400000;
  const cellW  = (isMobile ? CELL_W_MOB : CELL_W_DESK)[zoom];
  const nameW  = isMobile ? 110 : 200;
  const gridW  = cellW * days;   // EXACT pixel width — never changes
  ...
  const onTimelineScroll = () => {
    if (scrollRef.current && nameRef.current)
      nameRef.current.scrollTop = scrollRef.current.scrollTop;
  };

  if (!allTasks.length) return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', minHeight:0 }}>
      <GanttHeader isMobile={isMobile} zoom={zoom} startDay={startDay} endDay={endDay}
        onSetZoom={onSetZoom} onNav={onNav} onGoToday={onGoToday}/>
      <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--muted)', fontSize:'.88rem', border:'1px solid var(--border)', borderRadius:14, textAlign:'center', padding:40 }}>
        <div>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', minHeight:0 }}>
      ...
      {/* ── Grid ──────────────────────────────────────────────── */}
      <div style={{ flex:1, minHeight:0, display:'flex', border:'1px solid var(--border)', borderRadius:14, overflow:'hidden', background:'#0b0b13' }}>

        {/* Left: sticky name column */}
        <div ref={nameRef}
          style={{ width:nameW, flexShrink:0, overflowY:'hidden', borderRight:'1px solid var(--border)', background:'#0e0e1a' }}>
          {/* Header cell */}
          <div style={{ height:36, display:'flex', alignItems:'center', paddingLeft:10,
            fontSize:isMobile?'.6rem':'.66rem', fontWeight:700, textTransform:'uppercase',
            letterSpacing:.6, color:'var(--muted)', borderBottom:'1px solid var(--border)',
            background:'rgba(14,14,22,.97)', position:'sticky', top:0, zIndex:5, flexShrink:0 }}>
            Task
          </div>
          {/* Name rows */}
        ...
        <div ref={scrollRef} onScroll={onTimelineScroll}
          style={{ flex:1, overflowX:'auto', overflowY:'auto', position:'relative' }}>

          {/* Fixed-width inner — EXACTLY gridW, never stretches */}
          <div style={{ width:gridW, minWidth:gridW, maxWidth:gridW, position:'relative' }}>

            {/* Header: day columns */}
            <div style={{ height:36, display:'flex', position:'sticky', top:0, zIndex:5, background:'rgba(14,14,22,.97)', borderBottom:'1px solid var(--border)' }}>
              {colDates.map((d, i) => {
                ...
                const lbl  = zoom === 'week'
                  ? (isMobile ? `${['S','M','T','W','T','F','S'][day]}${date}` : `${['Su','Mo','Tu','We','Th','Fr','Sa'][day]} ${date}`)
                  : (date === 1 ? ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][d.getUTCMonth()] + ' ' : '') + date;
                return (
                  <div key={i} style={{
                    width:cellW, flexShrink:0,
                    display:'flex', alignItems:'center', justifyContent:'center',
                    fontSize:isMobile?'.5rem':zoom==='week'?'.65rem':'.58rem',
                    fontWeight:600, textTransform:'uppercase', letterSpacing:.5,
                    color: isT ? 'var(--accent)' : isWE ? 'rgba(255,255,255,.25)' : 'var(--muted)',
            ...
            {Object.values(groups).map((grp:Grp, gi) => [
              ...
              ...grp.tasks.map((t:any, ri:number) => {
                ...
                return (
                  <div key={`tr${gi}-${ri}`}
                    style={{ height:rowH, position:'relative', borderBottom:'1px solid rgba(255,255,255,.04)', background:'rgba(11,11,19,.7)' }}
                  ...
                  >
                    {/* Today line */}
                    {showToday && <div style={{ position:'absolute', top:0, bottom:0, left:todayOff, width:1, background:'rgba(255,95,160,.5)', zIndex:3, pointerEvents:'none' }}/>}
                    ...
                    {inView && (
                      <div onClick={() => onOpenDate(t.li,t.ti)}
                        style={{
                          position:'absolute',
                          top:barTop, left:leftPx,
                          width:widthPx, height:barH,
                          borderRadius:br,
                      ...
                      >
                        ...
                        {widthPx > (isMobile?28:40) && (
                          <div style={{ position:'absolute', left:6, right:4, top:0, bottom:0, display:'flex', alignItems:'center', fontSize:isMobile?'.52rem':'.62rem', color:'#fff', fontWeight:600, whiteSpace:'nowrap', overflow:'hidden', gap:3, zIndex:2 }}>
                            {t.text}
                            {prog>0 && !isMobile && <span style={{ fontSize:'.54rem', padding:'1px 4px', borderRadius:4, background:'rgba(0,0,0,.3)', flexShrink:0 }}>{prog}%</span>}
                          </div>
                        )}
                      </div>
                    )}
...
// ── Shared header ────────────────────────────────────────────────────
function GanttHeader({ isMobile, zoom, startDay, endDay, onSetZoom, onNav, onGoToday }:{
  isMobile:boolean; zoom:string; startDay:Date; endDay:Date;
  onSetZoom:(z:any)=>void; onNav:(d:number)=>void; onGoToday:()=>void;
}) {
  return (
