    ...
    parts.push(
      <a
        ...
        style={{
          ...
          color: "var(--accent)",
          textDecoration: "none",
          background: "rgba(111,95,255,.12)",
          border: "1px solid rgba(111,95,255,.3)",
          borderRadius: 5,
      ...
      >
        ...
        <i
          ...
          style={{ fontSize: ".55em", opacity: 0.6, flexShrink: 0 }}
        />
      </a>,
    );
...
export default function TaskCard({
  ...
  // Trigger border flash when task is checked
  const handleToggle = useCallback(() => {
    if (!t.done) {
      // about to be checked — flash
      setFlashing(true);
  ...
  const dur =
    t.startDate && t.endDate
      ? Math.max(
          1,
          Math.round(
            (new Date(t.endDate + "T00:00:00").getTime() -
  ...
  const badgeStyle = (
    type: "date" | "overdue" | "today" | "time" | "dur",
  ): React.CSSProperties => {
  ...
  return (
    <div
      ref={cardRef}
      className={flashing ? "task-flash" : undefined}
      ...
      onDragEnd={(e) =>
        (e.currentTarget as HTMLElement).classList.remove("task-dragging")
      }
      ...
      onTouchMove={(e) => {
        ...
        document
          .querySelectorAll(".touch-drop-target")
      ...
      onTouchEnd={(e) => {
        ...
        const elBelow = document.elementFromPoint(touch.clientX, touch.clientY);
        removeTouchGhost();
        const targetListEl = elBelow?.closest(
          "[data-li]",
        ) as HTMLElement | null;
        const targetTaskEl = elBelow?.closest(
          "[data-ti]",
        ) as HTMLElement | null;
        if (targetListEl) {
          const toLi = parseInt(targetListEl.dataset.li ?? "");
          const toTi = targetTaskEl
            ? parseInt(targetTaskEl.dataset.ti ?? "")
            : undefined;
          if (!isNaN(toLi)) {
            document.dispatchEvent(
              new CustomEvent("touch-move-task", {
                detail: {
                  fromLi: savedFromLi,
                  fromTi: savedFromTi,
                  toLi,
      ...
      onMouseEnter={(e) => {
        if (!t.done) {
          ...
          el.style.borderColor = "rgba(111,95,255,.3)";
          el.style.transform = "translateY(-1px)";
          el.style.boxShadow = "0 4px 16px rgba(0,0,0,.28)";
        }
      }}
      onMouseLeave={(e) => {
        const el = e.currentTarget;
        el.style.background = t.done ? "var(--done-bg)" : "var(--surface)";
        el.style.borderColor = t.done
          ? "rgba(255,255,255,.04)"
          : "var(--border)";
        el.style.transform = "";
        el.style.boxShadow = "";
    ...
    >
        ...
        >
          {t.done && (
            <i
              ...
              style={{ fontSize: ".56rem", color: "#fff" }}
            ></i>
          )}
        </div>
      ...
      {t.startDate || t.endDate || t.startTime || t.endTime || dur ? (
        <div
          style={{ display: "flex", flexWrap: "wrap", gap: 4, marginLeft: 25 }}
        >
          {t.startDate && (
            <span style={badgeStyle("date")}>
              <i className="fa-solid fa-play" style={{ fontSize: ".5rem" }}></i>{" "}
              {fmtDate(t.startDate)}
            </span>
          )}
          {t.endDate && (
            <span style={badgeStyle(endBadgeCls())}>
              <i className="fa-regular fa-calendar"></i> {fmtDate(t.endDate)}
            </span>
          )}
          {(t.startTime || t.endTime) && (
            <span style={badgeStyle("time")}>
              <i className="fa-regular fa-clock"></i> {t.startTime || ""}
              {t.startTime && t.endTime ? "–" : ""}
              {t.endTime || ""}
            </span>
          )}
          {dur && (
            <span style={badgeStyle("dur")}>
              <i className="fa-solid fa-ruler"></i> {dur}d
            </span>
          )}
        </div>
      ) : null}
          ...
          >
            <div
              style={{
                height: "100%",
                borderRadius: 3,
                background: "linear-gradient(90deg,#6f5fff,#ff5fa0)",
                transition: "width .4s",
                width: `${prog}%`,
              }}
            />
